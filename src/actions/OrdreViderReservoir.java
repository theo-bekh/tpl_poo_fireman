 ...
 *
 */
public class OrdreViderReservoir extends Evenement {
	 ...
	 * @param simulation
	 */
	public OrdreViderReservoir(long date, Carte carte, Robot robot, Incendie incendie, Simulateur simulation) {
		super(date, carte);
		this.robot = robot;
	...
	public String execute() {
		if (!robot.isOccupe() && this.isLegalEvent() && robot.getTankLevel() > 0) {
			...
			simulation.ajouteEvenement(new ViderReservoir(dateDeversement, this.getCarte(), robot, incendie, simulation.getData()));
	...
	public long calculDateDeversement() {
		long dateExecution = (long)(this.getDate() + 30000); //Le réservoir du drone prend 30 secondes à se vider.
